...
namespace ConsoleApp1.Utilty
{
    ...
    public static class DataConvert<T> where T : new()
    {
                        ...
                        if (dataFieldAttr != null)
                        {
                            ...
                            if (dr.Table.Columns.Contains(dataFieldAttr.ColumnName))
                            {
                                ...
                                propInfo.SetValue(entity,
                                                  ConvertHelper.ChangeType(dr[dataFieldAttr.ColumnName].ToString(), propInfo.PropertyType),
                                                  null);
                            }
        ...
        public static object ConvertEnumValue(string text)
        {
            if (Enum.IsDefined(typeof(T), text))
            {
                T enumValue = (T)Enum.Parse(typeof(T), text, true);
                return Convert.ChangeType(enumValue, typeof(Int16));
            }
            return text;
        }
    }
